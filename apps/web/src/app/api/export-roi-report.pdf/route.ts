...
function orgFromReq(req: NextRequest): string | undefined {
  try {
    ...
    return searchParams.get('org_id') || undefined;
  } catch {
    return undefined;
  }
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    if (!SERVICE_KEY) {
      return new Response('Service key missing', { status: 500 });
    }

    const qStart =
      (() => {
        try {
          return new URL(req.url).searchParams.get('quarter_start') || quarterStart();
        } catch {
          return quarterStart();
        }
      })();
...

    {
      ...
      if (used >= CAP) {
        ...
        return new Response('Monthly ROI report limit reached', {
          status: 402,
          headers: h,
        });
      }
