...
import { scimOk, scimErr } from "../../../scim/_scim_util.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Sorting is declared unsupported in ServiceProviderConfig; reject if present
    if (url.searchParams.has("sortBy") || url.searchParams.has("sortOrder")) {
      return scimErr(400, "Sorting not supported");
    }
    ...
    if (req.method === "PUT" || req.method === "PATCH") {
      ...
      if (error) return scimErr(400, error.message);
      return scimOk({ id: up.id, userName: up.user_name, externalId: up.external_id, active: up.active, emails: [{ value: up.email }] }, 200, { ETag: weakEtag(up.id, up.updated_at) });
    }
