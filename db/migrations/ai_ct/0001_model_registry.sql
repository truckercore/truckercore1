...
create index if not exists idx_ai_inf_key_time on ai_inference_events (model_key, created_at desc);
create index if not exists idx_ai_inf_corr on ai_inference_events (correlation_id);

-- Align with existing schema if table already exists
alter table ai_inference_events add column if not exists org_id uuid;
alter table ai_inference_events add column if not exists latency_ms int;
...
create policy ai_raw_service_1 on ai_inference_events for all to service_role using (true) with check (true);
create policy ai_raw_service_2 on ai_feedback_events  for all to service_role using (true) with check (true);
create policy ai_raw_service_3 on ai_training_jobs    for all to service_role using (true) with check (true);
...
as $$
declare
  m_id uuid; strat text; v_active uuid; v_ctrl uuid; v_cand uuid; v_a uuid; v_b uuid; pct int; choose int;
begin
  ...
  if strat is null or strat = 'single' then
    ...
    return v_active; -- shadow handled upstream
  end if;
end; $$;
