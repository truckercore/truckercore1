// Supabase Edge Function: prepare_counter
// Path: supabase/functions/prepare_counter/index.ts
// Invoke with: POST /functions/v1/prepare_counter
// Drafts a counter (dry-run) or enqueues an outbox job upon approval.
...
serve(async (req) => {
  ...
  try {
    const { org_id, load_id, target_cpm, dry_run = true, idem } = (await req.json()) as Req;
    if (!org_id || !load_id || !Number.isFinite(target_cpm)) return new Response("bad_request", { status: 400 });

    const preview = {
      ok: true,
      counter: { cpm: target_cpm, rationale: "Within market mid ±10%, HOS window OK" },
    } as const;
    ...
    if (dry_run) {
      try {
        await fetch(new URL("/functions/v1/slo_emit", req.url), {
          ...
          body: JSON.stringify({ route: "prepare_counter", latency_ms: Date.now() - t0, ok: true }),
        });
      } catch {}
    ...
    const job = await supa
      ...
      .insert({
        ...
        params: { type: "prepare_counter", load_id, target_cpm, idem: i },
        status: "queued",
    ...
    try {
      await supa.from("analytics_events").insert({
        org_id,
        event: "request",
        props: { load_id, target_cpm, idem: i },
        created_at: new Date().toISOString(),
      } as any);
    } catch {}
    ...
    try {
      await fetch(new URL("/functions/v1/slo_emit", (req as any).url ?? ""), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ route: "prepare_counter", latency_ms: Date.now() - t0, ok: false }),
      });
    } catch {}
