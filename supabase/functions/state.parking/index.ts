// Supabase Edge Function: state.parking
// GET /functions/v1/state.parking?bbox=west,south,east,north&min_conf=0.4
// Returns parking occupancy state for POIs within bbox with optional confidence threshold.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Select candidate POIs within bbox from either points_of_interest or pois if present
    // Prefer a unified query against points_of_interest (geo_lat, geo_lng, poi_type)
    // Fallback to pois(lat,lng,kind)

    // Try points_of_interest first
    ...
    try {
      const { data } = await admin
        ...
        .in("poi_type", ["truck_stop","parking"]); // treat truck_stop/parking as parking state hosts
    ...
    const { data: states } = await admin
      .from("parking_state")
      .select("poi_id, occupancy, confidence, last_update, source_mix")
      .in("poi_id", poiIds);
    ...
    const etag = `W/"ps-${paged.length}-${maxTs}"`;
    ...
    try { console.log(JSON.stringify({ event: 'endpoint', name: 'state.parking', items: paged.length, page, page_size, has_more, duration_ms })); } catch {}
