...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const required = ["invoice_number", "ref_number", "amount_cents", "status", "issued_at", "paid_at"];
    ...
    const rows = parsed
      .map((r) => ({
        ...
        amount_cents:
          r.amount_cents != null && r.amount_cents !== ""
            ? Number(String(r.amount_cents).replace(/[, ]/g, ""))
            : null,
